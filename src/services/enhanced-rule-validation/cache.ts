...
export function getCachedPreValidation(
  cacheKey: string,
): RuleValidationResult | null {
  return exceptionRuleCache.getNamespaced<RuleValidationResult>(
    NAMESPACE,
    cacheKey,
  );
}

export function setCachedPreValidation(
  cacheKey: string,
  value: RuleValidationResult,
): void {
