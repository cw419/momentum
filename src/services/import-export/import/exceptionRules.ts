...
export type ExceptionRuleImportData = Pick<ExceptionRule, ExceptionRuleImportFields>;

function isExceptionRuleType(value: unknown): value is ExceptionRuleType {
  return value === 'pause_only' || value === 'early_completion_only';
}

export function parseExceptionRulesToImport(exceptionRules: unknown): ExceptionRuleImportData[] {
