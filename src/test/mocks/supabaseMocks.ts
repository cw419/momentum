...
export const supabaseMockHandlers = [
  ...
  http.get(`${SUPABASE_URL}/rest/v1/chains`, ({ request }) => {
    ...
    const chains = Array.from(mockDatabase.chains.values()).filter(
      (chain: any) => !userId || chain.user_id === userId,
    );
  ...
  http.get(`${SUPABASE_URL}/rest/v1/active_sessions`, ({ request }) => {
    ...
    const sessions = Array.from(mockDatabase.active_sessions.values()).filter(
      (session: any) => !userId || session.user_id === userId,
    );
  ...
  http.get(`${SUPABASE_URL}/rest/v1/completion_history`, ({ request }) => {
    ...
    const history = Array.from(mockDatabase.completion_history.values()).filter(
      (record: any) => !userId || record.user_id === userId,
    );
  ...
  http.post(
    `${SUPABASE_URL}/rest/v1/completion_history`,
    async ({ request }) => {
      const body = (await request.json()) as any;
      const record = {
        ...body,
