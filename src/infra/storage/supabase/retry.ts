...
function toErrorWithMetadata(error: unknown): ErrorWithSupabaseMetadata {
  ...
  const wrapped: ErrorWithSupabaseMetadata = new Error(
    formatSupabaseError(error, String(error)),
  );
...
function isNonRetryableMessage(message: string): boolean {
  ...
  return NON_RETRYABLE_ERROR_MESSAGES.some((fragment) =>
...
function isNonRetryableCode(error: unknown): boolean {
  ...
  return (
    Boolean(errorCode) && NON_RETRYABLE_ERROR_CODES.has(errorCode as string)
  );
}
...
async function ensureAuthenticatedForRetry(
  ...
  const { user, isAuthenticated: isAuthenticatedAfterWait } =
    await deps.waitForAuthentication(5000);
...
export async function retryOperation<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (
        isNonRetryableMessage(lastError.message) ||
        isNonRetryableCode(error)
      ) {
        throw lastError;
      }
