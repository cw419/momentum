...
describe('storage/chains', () => {
  ...
  it('keeps active/deleted buckets separated and sorts deleted chains by deletion time desc', () => {
    ...
    expect(getDeletedChains().map((chain) => chain.id)).toEqual([
      'deleted-newer',
      'deleted-older',
    ]);
  });
  ...
  it('soft-deletes and restores root + descendants only', () => {
    ...
    expect(deletedIds).toEqual(
      expect.arrayContaining(['root', 'child', 'grand']),
    );
    ...
    expect(
      getActiveChains()
        .map((chain) => chain.id)
        .sort(),
    ).toEqual(['child', 'grand', 'outsider', 'root']);
  });
  ...
  it('restores only target subtree and keeps unrelated deleted chains untouched', () => {
    ...
    expect(
      getActiveChains()
        .map((chain) => chain.id)
        .sort(),
    ).toEqual(['restore-child', 'restore-root']);
  });
  ...
  it('cleans up expired deleted chains based on threshold', () => {
    ...
    saveChains([
      ...
      createUnitChain({
        id: 'keep-recent',
        deletedAt: new Date('2026-02-07T00:00:00.000Z'),
      }),
      createUnitChain({
        id: 'delete-old',
        deletedAt: new Date('2026-01-01T00:00:00.000Z'),
    ...
    expect(
      getChains()
        .map((chain) => chain.id)
        .sort(),
    ).toEqual(['keep-active', 'keep-recent']);
  });
