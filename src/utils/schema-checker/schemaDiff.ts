...
export function buildSchemaDiff(
  tableInfos: Record<string, TableInfo | null>,
): SchemaDiff {
  ...
  for (const [tableName, expectedColumns] of Object.entries(EXPECTED_SCHEMA)) {
    ...
    if (missing.length > 0) {
      ...
      logger.warn('SCHEMA', `表 ${tableName} 缺少列`, {
    ...
    if (extra.length > 0) {
      ...
      logger.info('SCHEMA', `表 ${tableName} 有额外列`, {
        extraColumns: extra,
      });
    }
  }
...
export function buildRecommendations(diff: SchemaDiff): string[] {
  ...
  const chainMigrationHints: Array<{
    columns: string[];
    recommendation: string;
  }> = [
    {
      columns: ['parent_id', 'type'],
      recommendation: '运行任务群支持迁移: 20250801160754_peaceful_palace.sql',
    },
    {
      columns: ['time_limit_hours', 'group_expires_at'],
      recommendation:
        '运行时间限制迁移: 20250808000000_add_group_time_limit.sql',
    },
    {
      columns: ['is_durationless'],
      recommendation:
        '运行无时长任务迁移: 20250808001000_add_durationless_flag.sql',
    },
    {
      columns: ['deleted_at'],
      recommendation: '运行软删除功能迁移: 20250814000000_add_soft_delete.sql',
    },
  ];
  ...
  if (
    diff.missingTables.includes('rsip_nodes') ||
    diff.missingTables.includes('rsip_meta')
  ) {
    recommendations.push(
      '运行RSIP功能迁移: 20250810000000_add_rsip_tables.sql',
    );
  }
