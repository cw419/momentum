...
export async function initializeRuleSystem(): Promise<{
  ...
  try {
    ...
    logger.info(
      'RULE_SYSTEM',
      `健康检查结果: ${healthReport.status} (${healthReport.score}/100)`,
    );

    if (healthReport.status === 'critical') {
      logger.warn('RULE_SYSTEM', '⚠️ 系统存在严重问题', {
    ...
    if (integrityReport.issues.length > 0) {
      logger.warn(
        'RULE_SYSTEM',
        `发现 ${integrityReport.issues.length} 个数据问题`,
      );
      ...
      if (autoFixableIssues.length > 0) {
        logger.info(
          'RULE_SYSTEM',
          `正在自动修复 ${autoFixableIssues.length} 个问题...`,
        );
        const fixResults =
