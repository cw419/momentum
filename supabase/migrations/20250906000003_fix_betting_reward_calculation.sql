-- ========================================
-- Fix Betting Reward Calculation
-- Date: 2025-09-06
-- Purpose: Fix the betting payout calculation to provide proper rewards
-- ========================================

-- Update the place_task_bet functions to set proper payout ratios
-- The current system only returns the bet amount, but should provide bet + reward

-- Drop and recreate the place_task_bet function with correct payout calculation
DROP FUNCTION IF EXISTS place_task_bet(uuid, uuid, integer, uuid);
DROP FUNCTION IF EXISTS place_task_bet(uuid, uuid, integer);

-- Create the corrected place_task_bet function
...
BEGIN
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'payout_ratio', '2x', -- Document the payout ratio
  ...
  VALUES (
    ...
    CASE 
      ...
      ELSE 'Placed bet (legacy mode - no write session) (2x payout potential)'
    END,
    new_bet_id
  );
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'payout_info', jsonb_build_object(
        'bet_amount', bet_amount,
        'potential_reward', bet_amount,
        'total_potential_payout', reward_payout
      )
    ),
    NOW()
  );
  ...
  result := jsonb_build_object(
    ...
    'payout_explanation', '赢得时获得: ' || bet_amount::text || '(返还押注) + ' || bet_amount::text || '(奖励) = ' || reward_payout::text || '积分'
  );
...
-- Also create the legacy version for backward compatibility
CREATE OR REPLACE FUNCTION place_task_bet(
  target_user_id uuid,
  target_session_id uuid,
  bet_amount integer
...
AS $$
BEGIN
  -- Call the main function with NULL write session token
  RETURN place_task_bet(target_user_id, target_session_id, bet_amount, NULL);
END;
$$;
...
-- Success message
SELECT '押注奖励计算修复完成! 现在成功完成任务将获得 2x 押注金额 (押注返还 + 同等奖励)' as result;