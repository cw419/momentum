...
完成区同时提供一个只读的单日时间轴。它使用 FullCalendar 的 `timeGridDay` 视图，可通过日期选择器和前后箭头浏览任意过去或未来日期；未来日期保持只读并显示为空。每个日期显示当天所有成功完成、且同时拥有 `startedAt` 和 `completedAt` 的会话；无论任务从今日计划还是任务链直接开启，都会进入时间表。迁移前、从今日计划完成而尚未写入完成历史开始时间的旧记录，会回退使用计划单元上保留的实际时间；新旧来源按完成记录去重。事件高度对应实际专注时长，并以 `✓ [目标项]` 或 `✓ [周期项]` 标注任务名称、起止时间和分钟数；已完成归档的目标项仍会保留在时间表中，真正删除的任务不会显示。完成记录中的描述和备注直接显示在事件中，点击事件可展开完整内容。时间轴覆盖 00:00–24:00，默认滚动到 06:00，移动端保持单列纵向滚动。
