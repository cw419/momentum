...
国策组现在可以组成无环的森林：每个组最多有一个父组，允许多个子组和多个根组。画布以独立分组框与父组到子组的箭头表达这一关系；组层级只用于组织与可视化，不改变容错或崩溃规则。

节点的父子关系和组归属必须一致：某节点及其全部后代始终继承该节点所在的国策组。创建子节点或将节点重新挂到新父节点下时，系统会采用父节点的组；跨组重挂会先提示，并在确认后迁移整个子树。若从节点编辑中把子节点迁入不同组，系统会提示该操作会解除原父子关系，并可选择同时建立相关组的从属关系。

组名称、Emoji 和容错值可直接从画布的组标签编辑。改名仅变更显示字段，组 ID 和既有节点、历史记录关联保持不变。

若连接的数据库尚未执行组层级迁移，普通国策组的创建和编辑仍可用；仅父子组关系不会保存，并会提示先执行迁移。
